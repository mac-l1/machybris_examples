...
int CopyBit::draw(rga_img_info_t *src, rga_img_info_t *dst, unsigned int flag)
{
    ...
    if(flag & FLAG_YUV2RGB_MASK) {
        ...
        if(Rga_Request.yuv2rgb_mode & 0x8 ) {
            Rga_Request.yuv2rgb_mode = (Rga_Request.yuv2rgb_mode & (~0x8)) | 0x01 << 4; 
	    //ALOGE("%s:  yuv2rgb_mode=%x\n", __FUNCTION__,Rga_Request.yuv2rgb_mode);
	    //Rga_Request.dst.yrgb_addr = Rga_Request.dst.uv_addr;
            //Rga_Request.dst.uv_addr = Rga_Request.dst.yrgb_addr +
            //            ((dst->vir_w + 15)&(~15)) * ((dst->vir_h+ 15)&(~15));
        }
    }
